...
Codex 用量小组件在设备上读取并计算 Codex 本地记录，以紧凑的圆环展示剩余用量、令牌、缓存、上下文占用和近期活动任务。会话文件始终保留在本机，小组件不会上传文件，也不会发送遥测数据。这是独立个人项目，不是 OpenAI 或 Codex 官方项目。显示结果为本地估算，可能与 OpenAI 官方账单或账户数据不同。
...
Codex 本地数据格式可能发生变化。界面中的限制、重置时间、任务活动和令牌统计均在本机计算，可能与 OpenAI 官方用量、账单或权益数据不同。
