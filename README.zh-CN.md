...
小组件不会扫描全盘、读取其他 Windows 用户、调用远程服务，也不会读取或保存账号凭据。它只读取界面展示所需的本机会话记录，并在当前用户自己的 `%LOCALAPPDATA%` 下写入偏好与累计缓存账本。
...
本项目通过本机 Codex 会话文件进行观测；如果文件格式发生变化，部分字段可能暂时无法识别。界面展示的限制、重置时间、任务活动和令牌统计均为尽力而为的本机观测，不是 OpenAI 官方用量、账单或权益数据。需要权威信息时，请以官方账户页面为准。
