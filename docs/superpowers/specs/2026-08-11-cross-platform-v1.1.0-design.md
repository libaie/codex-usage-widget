...
刷新使用进程隔离而不是尝试强制取消进程内文件 I/O。Windows 通过标准库 `System.Diagnostics.Process` 启动并复用一个由主进程托管的隐藏 Windows PowerShell `-ScanWorker` 子进程（`UseShellExecute=false`、`CreateNoWindow=true`），每轮只通过标准输入发送代次；进程异常、超时或数据目录切换时精确回收，下轮再启动。macOS 每次刷新通过 Foundation `Process` 运行同一个应用可执行文件的 `--scan-worker` 模式，不新增 helper target。主进程保留现有平台 UI 与生命周期控制，只有子进程访问 Codex 数据目录。worker 分支必须在 WPF/AppKit、托盘/菜单栏、实例锁、本地化 UI 和持久化状态初始化之前返回，不能为只读扫描加载界面程序集。

扫描子进程严格只读，不得写偏好、累计账本或 `reminders.json`。它只输出 schema v1 的规范化快照与逐会话令牌计数。生产端先校验字段/集合上限，并按 JSON 最坏转义后的 UTF-8 字节数计算保守上界；超过 256 KiB 时不序列化、不写结果。序列化后再校验精确字节数，只有不超过 256 KiB 才从本轮 `.tmp` 原子落为结果文件。主进程还要独立校验 worker 状态（macOS 还校验退出码）、文件大小、schema 和字段边界，之后才更新内存状态并作为唯一 writer 原子保存累计账本与提醒。数据目录与结果通道通过子进程环境/私有临时目录传递，不进入命令行或日志；每轮只创建一个精确临时目录，Windows ACL 仅允许当前用户，macOS 权限为 `0700`。成功、失败和超时都只清理本轮目录；启动时只清理应用专属根下、通过名称与所有者校验且超过 24 小时的孤儿目录，不跟随 reparse point/symlink。

每次扫描在第一次目录访问前启动独立的 12 秒自终止看门狗。Windows 持久 worker 写入本轮 ready 标记后启动 .NET timer，成功完成本轮即撤销并继续等待下一代次；macOS 子进程启动 Dispatch timer，正常退出时撤销。它们都不启动第二个 helper。Windows 冷启动到 ready 另有 30 秒硬上限，ready 后与 macOS 都受 10 秒用户预算约束；超时会终止精确 worker，macOS 最多再等 2 秒。Windows Job Object 还保证父进程消失时回收 worker，因此阻塞 I/O 没有无界孤儿生命周期；看门狗退出和父超时都不得产生可提交结果。
...
| 单次刷新 | ready 后使用单调时钟预算 10 秒；同一时刻只有一个只读扫描请求。到期立即把 UI 转为 stale/error 并精确终止 worker；Windows 下一 tick 创建替代 worker，macOS 按次创建新子进程，均不排队。 |
| worker 启动 | Windows 空输入 worker 从创建全新 PowerShell 5.1 进程到校验空结果，20 次的 p95 在固定 GitHub Windows 2022 发布环境不超过 750 ms；普通开发机因端点安全软件差异允许 3 秒。运行时冷启动硬上限仍为 30 秒；Mac p95 不超过 300 ms。持久 worker 就绪后的实际 30 文件刷新仍受 2 秒 p95 与 10 秒硬截止共同约束，不能用延长 15 秒刷新间隔掩盖回归。 |
| 扫描结果 | producer 在序列化前按字段/集合和最坏 UTF-8 JSON 转义预算拒绝超限，序列化后复核精确字节；结果文件始终不超过 256 KiB。主进程再次校验大小、worker 状态（macOS 含退出码）、schema、Int64 字符串、集合上限和稳定排序后才一次提交。无效、截断或超限按 `error`，不得更新账本或提醒。 |
...
| 扫描超时或异常退出 | 主进程只通过本轮持有的精确进程句柄终止并回收 worker，等待有界退出后清理本轮私有临时目录；扫描子进程无持久化权限，超时结果永不提交。 |
| 主进程崩溃/被强制结束 | Windows Job Object 回收托管 worker，活动扫描另有 12 秒每请求看门狗；macOS worker 自身 12 秒看门狗独立终止进程。本轮只读且结果不再有 writer，私有目录留给下次有界孤儿清理。 |
...
7. **CRITICAL — 进程隔离不能变成资源泄漏。** 最终 worker 必须通过空输入冷启动、30 个最大文件和连续 120 次刷新三档基准；另在阻塞 I/O 中强制结束父进程，worker 必须在 12 秒内被 Job/看门狗回收。达到启动、2 秒刷新、CPU/RSS、handle/fd 或残留门槛即失败，不得通过降低刷新频率、隐藏失败输出或改成脱离主进程监督的服务/守护进程绕过。
...
- 不为网络文件系统承诺内核级即时取消；通过精确终止隔离扫描子进程承诺 UI 预算、可信陈旧状态和下一周期恢复。Windows 持久 worker 仍由主进程 Job 托管，不增加脱离应用生命周期的后台服务。
...
| worker 生命周期 | Windows 复用一个由主进程托管的标准库子进程；macOS 每次刷新启动同一 app 的 worker 模式 | 进程内强停、外部服务/守护进程、`Start-Process -Wait` | 保留阻塞 I/O 隔离；Windows 避免重复冷启动，原生 Process 足够。 |
